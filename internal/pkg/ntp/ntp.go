...
type Syncer struct {
	...
	ntsSessions map[string]NTSSession
}
...
func (syncer *Syncer) SetTimeServers(timeServers []string) {
	...
	// Evict NTS sessions for servers no longer in the list.
	if syncer.UseNTS {
		for host := range syncer.ntsSessions {
			if !slices.Contains(timeServers, host) {
				delete(syncer.ntsSessions, host)
			}
		}
	}
...
func (syncer *Syncer) queryNTS(server string) (*Measurement, error) {
	...
	if err != nil {
		...
		syncer.timeServersMu.Lock()
		delete(syncer.ntsSessions, server)
...
func (syncer *Syncer) getNTSSession(server string) (NTSSession, error) {
	syncer.timeServersMu.Lock()
	session, ok := syncer.ntsSessions[server]
	syncer.timeServersMu.Unlock()
	...
	syncer.timeServersMu.Lock()
	syncer.ntsSessions[server] = session
	syncer.timeServersMu.Unlock()
